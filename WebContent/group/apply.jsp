...
<link rel="stylesheet" type="text/css" href="../css/group-apply.css">
<!-- 자바스크립트 -->
...
<script type="text/javascript" src="../js/group-apply.js"></script>
</head>
<body>
	<c:if test="${uid == null}">
						...
						<c:if test="${data.size() > 0}">
							<c:forEach var="i" begin="0" end="${data.size() - 1}">
								<li class="group" wid="${data.get(i).getWid()}">
									...
									<div class="buttons">
										<input class="exit" type="button" value="신청취소"/>
									</div>
								</li>
							</c:forEach>
						</c:if>
				...
				<footer id="article-footer">
					<div class="paging">
						<div class="paging-buttons"> 
							...
							<c:if test="${nowPage > 1}">
								<button type="button" value="1" href="apply.pr?page=1">처음</button>&nbsp;
								<button type="button" value="${nowPage - 1}" href="apply.pr?page=${nowPage - 1}">이전</button>&nbsp;
							</c:if>
							<c:forEach var="a" begin="${startPage}" end="${endPage}">
								...
								<c:if test="${a != nowPage}">
									<button type="button" value="${a}" href="apply.pr?page=${a}">${a}</button>&nbsp;
								</c:if>
							</c:forEach>
							...
							<c:if test="${nowPage < maxPage}">
								<button type="button" value="${nowPage + 1}" href="apply.pr?page=${nowPage + 1}">다음</button>&nbsp;
								<button type="button" value="${maxPage}" href="apply.pr?page=${maxPage}">마지막</button>
							</c:if>
						</div>
					</div>
				</footer>
