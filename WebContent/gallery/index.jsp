...
<link rel="stylesheet" type="text/css" href="../css/gallery-index.css">
<!-- 자바스크립트 -->
				...
				<article>
					<ul>
						<c:if test="${albumList != null && albumList.size() > 0}">
							<c:forEach var="i" begin="0" end="${albumList.size() - 1}">
								<li>
									<div class="background-image" style="background-image: url('../albumUpload/${albumList.get(i).getImg01()}'), url('../images/transparent.svg')">
									</div> 
									<a href="view.do${pid}${category}&aid=${albumList.get(i).getAid()}&page=${nowPage}">
										<span class="album-title">${albumList.get(i).getName()} | ${albumList.get(i).getTitle()}</span>
										<span class="album-writer">${albumList.get(i).getNick()}</span>
										<span class="album-write-date">${albumList.get(i).getWritedate()}</span>
									</a>
									<div class="album-bottom-info">
										<img alt="" src="../images/visibility_FILL0_wght400_GRAD0_opsz20.svg">
										${albumList.get(i).getReadCount()}
										<img alt="" src="../images/comment_FILL0_wght400_GRAD0_opsz20.svg">
										${albumList.get(i).getReplyCount()}
										<img alt="" src="../images/favorite_FILL0_wght400_GRAD0_opsz20.svg">
										${albumList.get(i).getRecommendCount()}
									</div>
								</li>
							</c:forEach>
						</c:if>
