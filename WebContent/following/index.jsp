				...
				<article>
					<div id="follow-page">
						...
						<ul class="follows" page="${nowPage}">
							<c:if test="${followingList != null && followingList.size() > 0}">
								<c:forEach var="i" begin="0" end="${followingList.size() - 1}">
									<li class="follow" pid="${followingList.get(i).getFollowing()}">
										<img src="../profileUpload/${followingList.get(i).getPhoto()}" onerror="this.onerror=null; this.src='../images/transparent.svg';" />
										<div class="follow-contents">
											<span class="follow-nick">${followingList.get(i).getNick()}</span>
											<span class="follow-date">
												${followingList.get(i).getFdate()}
											</span>
											<div class="follow-introduction">${followingList.get(i).getIntroduction()}</div>
											<c:if test="${followingList.get(i).getMyfollower() > 0}">
												<span class="follow-info">
													${followingList.get(i).getNick()}님은 나를 팔로우 하고 있습니다.
												</span>
											</c:if>
										</div>
										<c:if test="${followingList.get(i).getMyfollowing() > 0}">
